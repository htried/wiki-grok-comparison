#!/bin/bash
# Helper script to launch multiple GCP shard instances for edit requests fetching
# Usage: ./launch_edit_requests_shards.sh <NUM_SHARDS> [SHARD_SIZE] [START_OFFSET] [START_FROM_SHARD] [SLUGS_FILE]
# Example: ./launch_edit_requests_shards.sh 10 10000                    # 10 shards, 10k slugs each (default), starting from 0
# Example: ./launch_edit_requests_shards.sh 2 10000 0 0 gs://bucket/slugs.txt  # Use GCS slugs file
...
if [ -z "$1" ]; then
    ...
    echo "  $0 4 10000 0 0 hf                            # Use HuggingFace dataset"
    exit 1
fi

echo "Launching ${NUM_SHARDS} shards for edit requests fetching"
...
# Special case: "hf", "huggingface", or "hf_dataset" means use HuggingFace dataset
SLUGS_FILE_TO_PASS=""
if [ -n "$SLUGS_FILE_INPUT" ]; then
    # Check if it's a HuggingFace dataset identifier
    if [[ "$SLUGS_FILE_INPUT" =~ ^(hf|huggingface|hf_dataset)$ ]]; then
        SLUGS_FILE_TO_PASS="$SLUGS_FILE_INPUT"
        echo "Using HuggingFace dataset for slugs"
    elif [[ "$SLUGS_FILE_INPUT" == gs://* ]]; then
        SLUGS_FILE_TO_PASS="$SLUGS_FILE_INPUT"
    else
...
for ((SHARD_ID=$START_FROM_SHARD; SHARD_ID<$NUM_SHARDS; SHARD_ID++)); do
    ...
    if [ -n "$SLUGS_FILE_TO_PASS" ]; then
        ./scripts/gcp/gcp_edit_requests_shard_setup.sh ${SHARD_ID} ${START_IDX} ${END_IDX} "$SLUGS_FILE_TO_PASS"
    else
        ./scripts/gcp/gcp_edit_requests_shard_setup.sh ${SHARD_ID} ${START_IDX} ${END_IDX}
    fi
...
echo "Monitor with: gcloud compute instances list --filter='tags.items:edit-requests-scraper'"
echo ""
echo "To check logs for a specific shard:"
echo "  gcloud compute ssh edit-requests-shard-<ID> --zone=us-central1-a --command 'tail -f /var/log/edit-requests-scraper.log'"
