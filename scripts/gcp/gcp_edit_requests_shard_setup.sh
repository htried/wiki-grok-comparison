#!/bin/bash
# GCP Shard Setup Script for Edit Requests Fetcher
# This script sets up a shard of the edit requests fetcher on a GCP VM
# Usage: ./gcp_edit_requests_shard_setup.sh <SHARD_ID> <START_IDX> <END_IDX> [SLUGS_FILE(gs://... or path)]
# Example: ./gcp_edit_requests_shard_setup.sh 0 0 10000
...
INSTANCE_NAME="edit-requests-shard-${SHARD_ID}"
ZONE="us-central1-a"
...
echo "Setting up edit requests shard $SHARD_ID"
...
python3 \$REPO_DIR/scripts/gcp/edit_requests_runner.py --shard_id ${SHARD_ID} --start_idx ${START_IDX} --end_idx ${END_IDX} \$EXTRA_SLUGS_FLAG 2>&1 | tee /var/log/edit-requests-scraper.log
...
echo "View logs with: gcloud compute ssh ${INSTANCE_NAME} --zone=${ZONE} --command 'tail -f /var/log/edit-requests-scraper.log'"
