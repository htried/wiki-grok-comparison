...
# Usage: ./launch_shards.sh <NUM_SHARDS> <SHARD_SIZE> [START_OFFSET] [START_FROM_SHARD] [URLS_FILE] [SCRAPER_VERSION]
...
# Example: ./launch_shards.sh 5 1000 0 0 "" 0.2                         # Use v0.2 scraper
...
URLS_FILE_INPUT=${5:-}
SCRAPER_VERSION=${6:-0.2}

if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: $0 <NUM_SHARDS> <SHARD_SIZE> [START_OFFSET] [START_FROM_SHARD] [URLS_FILE] [SCRAPER_VERSION]"
    echo ""
    echo "Examples:"
    ...
    echo "  $0 5 1000 0 0 \"\" 0.2                         # Use v0.2 scraper (improved parsing)"
    exit 1
fi
...
echo "Scraper version: ${SCRAPER_VERSION}"
...
for ((SHARD_ID=$START_FROM_SHARD; SHARD_ID<$NUM_SHARDS; SHARD_ID++)); do
    ...
    if [ -n "$URLS_FILE_TO_PASS" ]; then
        ./scripts/gcp/gcp_shard_setup.sh ${SHARD_ID} ${START_IDX} ${END_IDX} "$URLS_FILE_TO_PASS" "$SCRAPER_VERSION"
    else
        ./scripts/gcp/gcp_shard_setup.sh ${SHARD_ID} ${START_IDX} ${END_IDX} "" "$SCRAPER_VERSION"
    fi
